...
function VideoPlayer({ file }: { file: StoredFile }) {
  ...
  return <div ref={frameRef} className="relative grid h-full place-items-center bg-[#141210]" onMouseMove={showControls} onClick={showControls}>
    <video ref={videoRef} key={file.key} autoPlay playsInline poster={file.posterUrl ?? undefined} className="max-h-full max-w-full" onPlay={() => { setPlaying(true); showControls(); }} onPause={() => { setPlaying(false); setControls(true); }} onTimeUpdate={(event) => setTime(event.currentTarget.currentTime)} onDurationChange={(event) => setDuration(event.currentTarget.duration || duration)} onError={() => setFailed(true)}><source src={file.url ?? ''} type={file.contentType ?? 'video/mp4'} />{file.contentType === 'video/quicktime' && <source src={file.url ?? ''} type="video/mp4" />}</video>
