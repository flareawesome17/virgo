          ...
          <li key={file.key} className="border-t border-[#d9cec5]/65 first:border-t-0 dark:border-white/[0.07]">
            ...
            >
              <span className={`grid size-10 place-items-center rounded-full ${active ? 'bg-[#b66a40] text-white' : 'bg-[#e9dfd7] text-[#6f5d52] dark:bg-white/[0.07] dark:text-white/55'}`}>
                {active && playing ? <Pause size={15} weight="fill" /> : <Play size={15} weight="fill" />}
              </span>
        ...
        <section aria-label="Album audio" className="mx-auto w-full max-w-5xl pb-40">
          {isLoading && files.length === 0 ? (
            ...
            <div className="py-24 text-center">
              <MusicNotesSimple size={36} weight="light" className="mx-auto text-[#9b877a]" />
        ...
        <aside className="fixed inset-x-3 bottom-3 z-40 mx-auto max-w-4xl rounded-[1.65rem] bg-[#1b1816]/96 p-1.5 text-white shadow-[0_24px_70px_-28px_rgba(58,38,27,0.72)] backdrop-blur-xl sm:inset-x-6">
          <div className="rounded-[1.3rem] border border-white/[0.08] bg-[#211d1a] px-4 py-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
            <div className="flex items-center gap-3">
              <button onClick={skipPrevious} aria-label="Previous track" className="grid size-9 place-items-center rounded-full text-white/70 hover:bg-white/[0.08] hover:text-white"><SkipBack size={18} weight="light" /></button>
              ...
              <button onClick={skipNext} aria-label="Next track" className="grid size-9 place-items-center rounded-full text-white/70 hover:bg-white/[0.08] hover:text-white"><SkipForward size={18} weight="light" /></button>
              ...
              <button onClick={() => setShuffle((value) => !value)} aria-pressed={shuffle} aria-label="Shuffle" className={`hidden size-9 place-items-center rounded-full sm:grid ${shuffle ? 'bg-white/[0.12] text-[#d89566]' : 'text-white/45 hover:text-white'}`}><Shuffle size={17} weight="light" /></button>
              <button onClick={cycleRepeat} aria-label={repeatLabel} className={`hidden size-9 place-items-center rounded-full sm:grid ${repeat !== 'off' ? 'bg-white/[0.12] text-[#d89566]' : 'text-white/45 hover:text-white'}`}><ArrowsClockwise size={17} weight="light" /></button>
